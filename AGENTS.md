...
- Walk/stand switch (`core/bridge.py`): `StandStillPolicy` runs the trained 94-dim `g1_stand` policy (metadata-driven gains/joint order, joints mapped by name); `WalkStandBridge` holds both policies, `--mode walk|stand|auto` (auto balances on zero command, no anchor), cross-fades targets over ~0.4 s. `--stand-policy` overrides the snapshot path, default is `find_latest_stand_policy()`.
