...
- Staged get-up v2 (`g1 recover` / `g1 gui --mode recover`): `GetUpBridge` sequences ROLL (any fall → supine) → GETUP (supine → stand) 94-dim policies via `core/getup_stages.py::StageSwitcher` (pure stdlib, shared with training through `training/getup/curriculum.py`). Facing = body-x projected gravity (yaw-invariant: supine ≈ −1, prone ≈ +1); ROLL gate (face-up + low + slow + RMS supine err < 0.30) keeps the handoff inside GETUP's start distribution; GETUP gate is the standing handoff (0.72/0.30/0.12). Supine falls skip ROLL; timeouts re-roll; standing shortcut jumps to DONE. Train with `g1 train -- --task getup --stage roll|standup` (only 2 getup tasks + stand; v1 A/B/C removed). Dashboard metric per stage is `STAGE_METRICS` in `lab/train.py` (must name a real reward); dashboard charts come from `CHART_GROUPS` in `lab/dashboard.py`.
